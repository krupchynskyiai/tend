...
async def init_mongodb(*_):
    ...
    DB.default_region = "Закарпатська область"    # Хардкод значення регіону
...
async def create_test_tender():
    # Створення тестового тендеру з використанням захардкодженого регіону
    tender_data = {
        "procuringEntityRegion": DB.default_region,  # Використовуємо захардкоджений регіон
        # Інші дані тендеру...
    }
    await save_tender("TEST_TENDER_ID", tender_data)

# Виклик функції для створення тестового тендеру
async def main():
    await create_test_tender()

# Виклик асинхронної функції main
asyncio.run(main())